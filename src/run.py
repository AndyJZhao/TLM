...
def preprocess(args, model, tokenizer, raw_datasets, num_labels, label_list, logger, accelerator):
    ...
    def preprocess_function(examples):
        ...
        result = tokenizer(examples["text"], padding=padding, max_length=args.max_length, truncation=True,
                           return_special_tokens_mask=True, )
...
def main():
    ...
    train_dataset, eval_dataset, test_dataset, data_collator, eval_data_collator = preprocess(args, model, tokenizer,
    ...
    if args.external_ratio > 1:
        ...
        external_data_collator = DataCollatorForLanguageModeling(tokenizer=tokenizer,
                                                                 mlm_probability=args.mlm_probability)
        external_dataloader = DataLoader(
            external_dataset, shuffle=True, collate_fn=external_data_collator,
    ...
    eval_dataloader = DataLoader(eval_dataset, collate_fn=eval_data_collator,
